...
SELECT * FROM public.naep LIMIT 50
...
SELECT state, count(avg_math_4_score) AS cnt,
...
SELECT public.naep.state, round(avg_math_4_score, 2) AS average_math_4, total_expenditure
FROM public.naep
LEFT JOIN public.finance ON public.naep.id = public.finance.id
WHERE avg_math_4_score is not null AND public.naep.year = 2000
